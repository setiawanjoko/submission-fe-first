...
class NoteItem extends HTMLElement {
  ...
  connectedCallback() {
    ...
    archive.innerHTML = JSON.parse(this.getAttribute("notes-archived"))
      ? "Activate"
      : "Archive";
  ...
  archive() {
    ...
    if (this.getAttribute("notes-archived") == "true") {
      document.dispatchEvent(
        new CustomEvent(UNARCHIVE_EVENT, { detail: { id } }),
      );
    } else {
      document.dispatchEvent(
        new CustomEvent(ARCHIVE_EVENT, { detail: { id } }),
      );
    }
  }
  ...
  destroy() {
    ...
    let isDestroyed = document.dispatchEvent(
      new CustomEvent(DELETE_EVENT, { detail: { id } }),
    );
